...
| R-008 | 业务模块颗粒度不当或循环依赖导致无法复用 / 升级困难 | Medium | Medium | `Open`     | TBD    | 依赖图 + lint 约束（RFC）；业务包仅依赖公开 API；代码评审 checklist | ADR-0004 |
...
- **D-005**：业务能力模块化 — 首批冻结的域、包名与 `peerDependencies` 策略（OpenAPI/内部接口是否进包）— 建议在 RFC 中结论，截止不晚于 **M2 启动周**
